...
from opening_messages import first_time_opening_messages
from persona_cloning.enums.phase import Phase
from persona_cloning.models.persona import PersonaScores
...
class PersonaCloneAIPartner:
    ...
    ### ❌❌ Not needed anymore
    # def initialize_persona_scores(self) -> PersonaScores:
    ...
    #     return PersonaScores.from_dict(data=persona_scores)
    ...
    #     return {**base.to_dict() , **phase_adaptations[self.current_phase]}

    ## Lots of work needed to be done.
    # def _initiate_first_conversation(self) -> str:
    ...
    def initiate_first_conversation_again(self)->str:
        # Track this as beginning of emotional DNA extraction
        # self.track_dimension_exploration("emotional_dna", "initial_mood_assessment")
    ...
    async def build_response_components(self, user_message: str, emotional_state: str, target_dimension: str) -> str: # Dict:
        ...
        prompt = (
            f"You are a caring AI partner in a conversation. "
        ...
        # Example: response_json = await my_llm.generate_json(prompt)
        # For demonstration, fallback to static values if LLM is not available
        try:
            # response_json = await my_llm.generate_json(prompt)
            response = await slm.ainvoke(prompt, model="gpt-3.5-turbo", max_tokens=300, temperature=0.7)
        except Exception:
            # Fallback to static/dummy values
            # response_json = {
            #     "validation": self.generate_validation(user_message, emotional_state),
            #     "affection": self.generate_affection_expression(),
            #     "curiosity": self.generate_strategic_question(target_dimension),
            ...
            # I don't want to use the static values anymore.
            raise
    ...
    def generate_affection_expression(self) -> str:
        ...
        affection_level = self.total_personality_phase.affection_level
        affection_terms = self.current_personality.affection_expressions
    ...
    def add_personality_element(self) -> str:
        ...
        # }
        personality_touches = {
            Phase.MIRROR.mood: ["💕", "Sweet soul", "My heart goes out to you"],
            "playful": ["😉", "You're adorable!", "I can't help but smile!"],
            "energetic": ["🔥", "This is so exciting!", "I'm buzzing with energy!"],
            "wise": ["💭", "That's so insightful", "You have such depth"],
            "romantic": ["✨", "You're incredible", "My heart skips a beat"]
    ...
    def determine_current_personality_mood(self) -> str:
        ...
        return self.current_phase.mood
    ...
    def calculate_phase_readiness_bonus(self, catalyst: Dict) -> float:
        ...
        # }
        phase_readiness_scores:dict[Phase, float] = {
            Phase.MIRROR: 10,  # Mirror phase - always ready
            Phase.GENTLE_CHALLENGER: self.assess_challenge_readiness_score(),
            Phase.COMPLEMENTARY_OPPOSITE: self.assess_contrast_readiness_score(), 
            Phase.GROWTH_CATALYST: self.assess_growth_readiness_score(),
            Phase.AUTHENTIC_COMPANION: self.assess_companion_readiness_score()
        }
        
        return phase_readiness_scores.get(self.current_phase, 5) #👉🤔 I think it's Default bonus for unknown phases
    ...
    def assess_challenge_readiness_score(self) -> float:
        ...
        trust_score = self.persona_scores.emotional_dna.score
    ...
    def assess_contrast_readiness_score(self) -> float:
        ...
        personality_score = self.persona_scores.personality_authenticity.score
        communication_score = self.persona_scores.communication_blueprint.score
    ...
    def assess_growth_readiness_score(self) -> float:
        ...
        growth_score = self.persona_scores.growth_challenge_threshold.score
        relationship_score = self.persona_scores.relationship_readiness.score 
    ...
    def calculate_user_comfort_level(self) -> float:
        ...
        comfort_indicators = [
            ...
            self.persona_scores.emotional_dna.vulnerability_comfort_zone,
            self.persona_scores.communication_blueprint.conversation_rhythm,
    ...
    def select_optimal_topic(self, topic_options: List[Dict]) -> Dict:
        ...
        dimension_scores = {dim: scores["score"] for dim, scores in self.persona_scores.to_dict().items()}
    ...
    ## ❌❌ UNUSED FOR NOW ----- This is technically hardcoded.
    ...
    ## 1️⃣ Question arises: Why am I even considering emotional state. What shall I tend to achieve?
    # Because I can't be answering everytime using LLM.
    # 👉👉 I can use emotions to understand and give direction and purpose to the chats.
    ## Not using this for now, but keeping it here for future reference.
    # ❌❌ UNUSED FOR NOW
    # def detect_emotional_state_go_emotions(self, user_message):
    #     """Detect user's emotional state for appropriate caring response"""
    #     model_outputs:list[Any] = emotion_classifier(user_message)
    ...
    def update_dimension_score(self,  dimension: str, specific_area: str): ## Specific area should not be string
        ...
        total_points = (base_points + depth_bonus)* self.current_phase.phase_multiplier
        ...
        # if specific_area in self.persona_scores.:
        self.persona_scores.get_dimension(dimension).add_attribute_score(specific_area, total_points)
    ...
    def calculate_dimension_total_score(self, dimension: str):
        ...
        ## ------------ replaced by a method in PersonaScores class -- start
        # dimension_data = self.persona_scores.get_dimension(dimension).to_dict()
        ...
        ## ------------ replaced by a method in PersonaScores class -- end

        total_score = self.persona_scores.get_dimension(dimension).total_attributes_score()
        ...
        if self.persona_scores.get_dimension(dimension).is_all_attributes_greater_than_0():
            total_score *= 1.1  # 10% completion bonus
        
        # Cap at 100
        # self.persona_scores[dimension]["score"] = min(100, total_score)
        self.persona_scores.set_score(dimension, min(100, total_score))
    ...
    def calculate_overall_completion_score(self) -> float:
        ...
        weights = {
            ...
            Dimension.EMOTIONAL_DNA: 0.18,
            Dimension.COMMUNICATION_BLUEPRINT: 0.16,
            Dimension.VALUES_BELIFE_SYSTEM: 0.15,
            Dimension.PERSONALITY_AUTHENTICITY: 0.14,
            Dimension.RELATIONSHIP_READINESS: 0.13,
            Dimension.LIFE_COMPATIBILITY_MATRIX: 0.10,
            Dimension.GROWTH_CHALLENGE_THRESHOLD: 0.08,
            Dimension.INTUITIVE_CONNECTION: 0.06
        }
        
        weighted_score = sum(
            # self.persona_scores[dim]["score"] * weight
            self.persona_scores.get_dimension(dim).score * weight
    ...
    def calculate_consistency_bonus(self) -> float:
        ...
        dimension_scores = [
            ...
            dimension.score for dimension in self.persona_scores.dimensions
        ]
    ...
    def get_priority_dimension(self) -> str:
        ...
        importance_weights: dict[Dimension, float] = {
            # "emotional_dna": 1.2,
            ...
            Dimension.EMOTIONAL_DNA: 1.2,
            Dimension.COMMUNICATION_BLUEPRINT: 1.1,
            Dimension.VALUES_BELIFE_SYSTEM: 1.0,
            Dimension.RELATIONSHIP_READINESS: 1.15,
            Dimension.PERSONALITY_AUTHENTICITY: 1.0,
            Dimension.LIFE_COMPATIBILITY_MATRIX: 0.9,
            Dimension.GROWTH_CHALLENGE_THRESHOLD: 0.8,
            Dimension.INTUITIVE_CONNECTION: 0.7
        }
        ...
        for dimension, scores in self.persona_scores.to_dict().items():
            # Lower score = higher priority, adjusted by importance weight
            priority_score = (100 - scores["score"]) * importance_weights.get(Dimension(dimension), 1.0)
            dimension_priorities[dimension] = priority_score
