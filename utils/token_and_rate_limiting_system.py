...
class UserUsage(BaseModel):
    ...
    hour_reset_time: datetime = Field(default_factory=lambda: datetime.utcnow() + timedelta(hours=1))
    day_reset_time: datetime = Field(default_factory=lambda: datetime.utcnow() + timedelta(days=1))
...
class UsageTracker:
    ...
    async def track_request(self, user_id: str, request_data: Dict[str, Any]) -> bool:
        """
    ...
    async def get_or_create_usage(self, user_id: str) -> UserUsage:
        ...
        if not usage_dict:
            ...
            await self.collection.insert_one(usage.dict())
            return usage
